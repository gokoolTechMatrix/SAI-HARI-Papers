...
/**
 * EMPLOYEE PROFILE – FULL SUITE (Pastel Red/Blue Theme + Advanced Attendance Calendar UX)
...
function Section({ title, desc, children }: { title: string; desc?: string; children: React.ReactNode }) {
  return (
    <Card className="rounded-2xl border border-[var(--pastel-border)] shadow-sm">
  ...
  return (
    <Section title="Employee Details" desc="Comprehensive form (photo + 40+ fields)">
      ...
      <CardFooter className="justify-end pt-4">
        ...
        <Button className="ml-2 bg-[var(--blue-600)] text-white hover:opacity-90">Save Details</Button>
      </CardFooter>
    </Section>
  );
}
...
function AttendanceCalendar() {
  ...
  function marker(day: Date) {
    ...
    const classes: Record<string, string> = {
      P: "bg-[var(--pastel-blue)] text-blue-900",
      A: "bg-[var(--pastel-red)] text-red-900",
  ...
  return (
    <Section title="Attendance" desc="Pastel calendar with quick marking & summaries">
          ...
          <div className="rounded-xl border bg-white p-1 text-xs md:text-sm">
            <div className="flex overflow-hidden rounded-lg">
              <button
                ...
                className={`px-3 py-1 ${selectedStatus === "P" ? "bg-[var(--pastel-blue)]" : ""}`}
              >Present</button>
              <button
                type="button"
                onClick={() => setSelectedStatus("A")}
                className={`px-3 py-1 border-l ${selectedStatus === "A" ? "bg-[var(--pastel-red)]" : ""}`}
              >Absent</button>
              <button
                type="button"
                onClick={() => setSelectedStatus("L")}
                className={`px-3 py-1 border-l ${selectedStatus === "L" ? "bg-yellow-200" : ""}`}
              >Leave</button>
              <button
                type="button"
                onClick={() => setSelectedStatus("")}
                className={`px-3 py-1 border-l ${selectedStatus === "" ? "bg-slate-200" : ""}`}
              >Clear</button>
            </div>
          </div>
        ...
        <div className="mt-4">
          <div className="mb-2 flex items-center justify-between text-xs md:text-sm">
            <div className="flex flex-wrap gap-2">
              <Badge className="bg-[var(--pastel-blue)] text-blue-900">Present {counts.P}</Badge>
              <Badge className="bg-[var(--pastel-red)] text-red-900">Absent {counts.A}</Badge>
              <Badge className="bg-yellow-200 text-yellow-900">Leave {counts.L}</Badge>
            </div>
            <div className="text-slate-900">Marked: {totalMarked}/{dayKeys.length}</div>
          </div>
          {/* Progress bar */}
          <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200">
            <div className="h-2 bg-[var(--pastel-blue)]" style={{ width: `${(counts.P / dayKeys.length) * 100}%` }} />
          </div>
        </div>
      </div>
    </Section>
  ...
  return (
    <Section title="Salary Sheet" desc="Auto-calculated from attendance & rules">
      ...
      <CardFooter className="justify-end pt-4">
        ...
        <Button className="ml-2 bg-[var(--blue-600)] text-white hover:opacity-90">Generate Slip</Button>
      </CardFooter>
    </Section>
  );
}
  ...
  return (
    <Section title="Advance Copy" desc="For employee advances & EMI">
      ...
      <CardFooter className="justify-end pt-4">
        ...
        <Button className="ml-2 bg-[var(--blue-600)] text-white hover:opacity-90">Save Advance</Button>
      </CardFooter>
    </Section>
  );
}
  ...
  return (
    <Section title="Bank Copy" desc="Printable bank statement for salary disbursal">
      ...
      <CardFooter className="justify-end pt-4">
        ...
        <Button className="ml-2 bg-[var(--blue-600)] text-white hover:opacity-90">Save Bank Copy</Button>
      </CardFooter>
    </Section>
  );
}
...
export default function EmployeeFullSuitePage() {
  return (
    <div className="mx-auto max-w-[1400px] min-h-screen bg-[var(--pastel-bg)] p-0">
      {/* Global pastel theme tokens */}
      <style jsx global>{`
        :root {
          --pastel-red: #fecaca; /* softer pastel red */
          --pastel-blue: #cfe8ff; /* softer pastel blue */
          --pastel-bg: #f6f7fb;   /* clean white/grey base */
          --pastel-border: #e6e9ef; /* subtle borders */
          --blue-600: #2563eb; /* action color */
        }
      `}</style>

      {/* Pastel gradient app bar */}
      <div className="sticky top-0 z-10 mb-6 shadow-sm">
        <div className="-mx-0 bg-gradient-to-r from-[var(--pastel-blue)] via-white to-[var(--pastel-red)] px-6 py-4">
          <div className="mx-auto flex max-w-[1400px] items-center justify-between">
            <div>
              <div className="text-xs text-slate-900">Employees / {EMP.name}</div>
              <h1 className="text-2xl font-semibold text-slate-900">Profile & Operations</h1>
            </div>
            <div className="flex items-center gap-2">
              <Button variant="outline" className="bg-white/60 backdrop-blur">Preview</Button>
              <Button className="bg-[var(--blue-600)] text-white hover:opacity-90">Save</Button>
            </div>
          </div>
        </div>
      </div>
            ...
            <Section title="Snapshot" desc="Quick info">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-slate-900">{EMP.role} • {EMP.dept} • {EMP.type}</div>
                  <div className="text-xs text-slate-900">{EMP.id}</div>
                </div>
              </div>
              <Separator className="my-4" />
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-xl bg-white p-3 shadow-sm text-slate-900">Leave Balance<br/><span className="text-lg font-semibold">12 days</span></div>
                <div className="rounded-xl bg-white p-3 shadow-sm text-slate-900">On-Time %<br/><span className="text-lg font-semibold">92%</span></div>
              </div>
            </Section>
          </div>
...
<div className="mt-6 text-center text-xs text-slate-900">By continuing you agree to the Terms & Privacy.</div>
